...
export async function exec(
	ssh: NodeSSH,
	command: string,
	options: { cwd?: string; showStdout?: boolean } = {
...
${command}`,
		{
			cwd: options.cwd,
			onStdout: (chunk) =>
				(config.verbose || options.showStdout) && process.stdout.write(chunk),
			onStderr: (chunk) =>
				(config.verbose || options.showStdout) && process.stderr.write(chunk),
			execOptions: {
				pty: true,
			},
			stdin: process.stdin,
